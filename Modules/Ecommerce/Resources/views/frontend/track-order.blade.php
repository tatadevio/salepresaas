...
<div class="breadcrumb-section">
    <div class="container">
        <div class="row">
            <div class="col">
                <h1 class="page-title">{{trans('file.Track Order')}}</h1>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-6 offset-md-3">
                ...
                <div class="order-details card mb-5">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <span class="mute">{{trans('file.Time')}} : </span>
                                    <span>{{ date('d-m-Y h:i:s', strtotime($sale->created_at)) }}</span>
                                </div>
                                <div class="mb-3">
                                    <span class="mute">{{trans('file.Order Status')}} : </span>
                                    @if($sale->sale_status == 1)
                                    <span>{{trans('file.Complete')}}</span>
                                    @elseif($sale->sale_status == 2)
                                    <span>{{trans('file.Pending')}}</span>
                                    @endif
                                </div>
                                <div class="mb-3">
                                    <span class="mute">{{trans('file.Payment Method')}} : </span>
                                    <span>{{ $sale->payment_mode }}</span>
                                </div>
                                <div class="mb-3">
                                    <span class="mute">{{trans('file.Payment Status')}} : </span>
                                    @if($sale->payment_status == 1)
                                    <span>{{trans('file.Pending')}}</span>
                                    @elseif($sale->payment_status == 4)
                                    <span>{{trans('file.Paid')}}</span>
                                    @endif
                                </div>
                                <div class="mb-3">
                                    @if($delivery != 0)
                                    <span class="mute">{{trans('file.Shipping Status')}} : </span>
                                    @if($sale->payment_status == 1)
                                    <span>{{trans('file.Packing')}}</span>
                                    @elseif($sale->payment_status == 2)
                                    <span>{{trans('file.Delivering')}}</span>
                                    @elseif($sale->payment_status == 3)
                                    <span>{{trans('file.Delivered')}}</span>
                                    @endif
                                    @else
                                    <span class="mute">{{trans('file.Shipping Status')}} : </span>
                                    <span>{{trans('file.Pending Confirmation')}}</span>
                                    @endif
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <span class="mute">{{trans('file.Name')}} : </span>
                                    <span>{{ $sale->shipping_name }}</span>
                                </div>
                                <div class="mb-3">
                                    <span class="mute">{{trans('file.Phone')}} : </span>
                                    <span>{{ $sale->shipping_phone }}</span>
                                </div>
                                <div class="mb-3">
                                    <span class="mute">{{trans('file.Address')}} : </span>
                        ...
                        <div class="cart-table table-content table-responsive">
                            <table class="table">
                                ...
                                <tfooter>
                                    <tr>
                                        <td></td>
                                        <td><span class="mute">{{trans('file.Shipping')}}</span></td>
                                        <th>(+) {{$currency->symbol ?? $currency->code}}{{ $sale->shipping_cost ?? '0' }}</th>
                                    </tr>
                                    <tr>
                                        <td></td>
                                        <td><span class="mute">{{trans('file.Discount')}}</span></td>
                                        <th>(-) {{$currency->symbol ?? $currency->code}}{{ $sale->coupon_discount ?? '0' }}</th>
                                    </tr>
                                    <tr>
                                        <th></th>
                                        <th><h4>{{trans('file.Total')}}</h4></th>
                                        <th><h4>{{$currency->symbol ?? $currency->code}}{{ ($sale->grand_total - $sale->coupon_discount) }}</h4></th>
                                    </tr>
                                </tfooter>
                            </table>
                        </div>
                ...
                <a href="{{url('track-order')}}" class="button lg style1 d-block text-center mt-5">{{trans('file.Track another order')}}</a>
                @else
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-sm-6 col-sm-12">
                                <div class="res-box">
                                    <button type="submit" tabindex="4" class="button style1 d-block">{{trans('file.Track')}}</button>
                                </div>
                            </div>
                        </div>
                    </div>
